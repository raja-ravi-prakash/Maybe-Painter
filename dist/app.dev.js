...
function updateFrame() {
  shapes.forEach(function (el) {
    ...
    if (el.type) cont.strokeRect(el.x, el.y, el.width, el.height);else cont.fillRect(el.x, el.y, el.width, el.height);
  });
}
...
function detectRect(cx, cy) {
  ...
  for (var i = 0; i < shapes.length; i++) {
    var _shapes$i = shapes[i],
        x = _shapes$i.x,
        y = _shapes$i.y,
        width = _shapes$i.width,
        height = _shapes$i.height;
    var flag = false;
...
can.addEventListener("pointerdown", function (e) {
  isCon = true;
  if (drm) addRect(e.clientX, e.clientY);else detectRect(e.clientX, e.clientY);
});
can.addEventListener("pointermove", function (e) {
  if (isCon) {
    ...
    if (drm) updateRect(x, y);else dragRect(x, y);
